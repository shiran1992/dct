...
function export_csv(file_path, sheet_cnt)
	...
	for filename in popen:lines() do
		if (string.sub(filename,-4) == ".csv") then
			os.execute("iconv -f CP936 -t UTF-8 tmp/" .. filename .. " > tmp/" .. filename .. "x")
		end
	end
...
function output_json(t, f, sign, tabs, comma, w_tbl)
	if not (type(t)=="table") then
		print("Invalid table to output!")
		return
	end
	...
	for _, k in ipairs(k_tbl) do
		...
		local isEnd = _ == #k_tbl
		local dh = isEnd and "" or ","
		local tv = type(v)
		if tv=="table" then
			f:write(tabs, k)
			output_json(v, f, nil, tabs .. "	", true and (not isEnd))
		elseif tv=="string" then
			if v:sub(1,1)=="\"" and v:sub(-1)=="\"" then
				f:write(tabs, k, v, ","..connector)
			else
				f:write(tabs, k, "\""..v.."\"", dh..connector)
			end
		elseif tv=="boolean" then
			f:write(tabs, k, tostring(v), dh..connector)
		else
			f:write(tabs, k, v or 0, dh..connector)
		end
	end
	if not (tabs=="")then
		tabs = tabs:sub(1,-2)
...
for k in letterstring:gfind("%a") do
	...
	local k3 = 'b'..k
	meta_key[k3] = idx + 52
	meta_key[k3:upper()] = idx + 52

	local k4 = 'c'..k
	meta_key[k4] = idx + 78
	meta_key[k4:upper()] = idx + 78
